...
export function WorkshopDashboard() {
 ...
 return <SidebarProvider><Sidebar collapsible="icon"><SidebarHeader className="p-4"><Link href="/dashboard" className="flex items-center gap-3 font-bold"><Wrench className="text-primary"/>Auto-Repair Shop</Link></SidebarHeader><SidebarContent><SidebarGroup><SidebarGroupLabel>Workspace</SidebarGroupLabel><SidebarGroupContent><SidebarMenu>{availableNav.map(item=><SidebarMenuItem key={item.id}><SidebarMenuButton tooltip={item.label} isActive={active===item.id} onClick={()=>navigate(item.id)}><item.icon/><span>{item.label}</span></SidebarMenuButton></SidebarMenuItem>)}</SidebarMenu></SidebarGroupContent></SidebarGroup></SidebarContent><SidebarFooter className="p-4"><p className="truncate font-semibold">{session.account.fullName}</p><p className="capitalize text-sm text-muted-foreground">{role}</p><form action="/auth/sign-out" method="post"><Button variant="outline" type="submit"><LogOut/>Sign out</Button></form></SidebarFooter></Sidebar><SidebarInset>
