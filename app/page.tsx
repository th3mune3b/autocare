    ...
    <main className="min-h-screen bg-[#f6f7f8] text-[#182027]">
      <header className="sticky top-0 z-30 border-b border-black/5 bg-[#f6f7f8]/90 backdrop-blur-xl">
        <div className="mx-auto flex h-18 max-w-7xl items-center justify-between px-5 lg:px-8">
          <Link href="/" className="flex items-center gap-3"><div className="grid size-10 place-items-center rounded-xl bg-primary text-white shadow-[0_8px_25px_rgba(241,90,36,.25)]"><Wrench className="size-5" /></div><div><p className="font-extrabold tracking-tight">AutoCare Pro</p><p className="text-[11px] text-muted-foreground">Workshop Management System</p></div></Link>
      ...
      <section className="relative overflow-hidden border-b border-black/5">
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-12 px-5 py-20 lg:grid-cols-[.86fr_1.14fr] lg:px-8 lg:py-28">
          <div><div className="inline-flex items-center gap-2 rounded-full border border-primary/20 bg-white px-3 py-1.5 text-xs font-bold text-primary shadow-sm"><ShieldCheck className="size-4" /> One secure workshop workspace</div><h1 className="mt-6 max-w-2xl text-5xl font-black leading-[1.03] tracking-[-.045em] sm:text-6xl">Every repair, part and payment—under control.</h1><p className="mt-6 max-w-xl text-lg leading-8 text-muted-foreground">AutoCare Pro connects customers, service advisors, mechanics and management across the complete vehicle-service lifecycle.</p><div className="mt-8 flex flex-wrap gap-3"><Button size="lg" asChild className="h-12 rounded-xl px-6"><Link href="/sign-up">Set up your account <ArrowRight /></Link></Button><Button size="lg" variant="outline" asChild className="h-12 rounded-xl bg-white px-6"><Link href="/dashboard">Open workspace</Link></Button></div><div className="mt-9 flex flex-wrap gap-x-6 gap-y-3 text-sm font-semibold text-muted-foreground">{["Role-based access", "Complete audit trail", "Single-branch focused"].map(item => <span key={item} className="flex items-center gap-2"><CheckCircle2 className="size-4 text-emerald-600" />{item}</span>)}</div></div>

          <div className="relative rounded-[28px] border border-black/10 bg-[#172028] p-3 shadow-[0_35px_80px_rgba(19,28,34,.2)]"><div className="rounded-[20px] bg-[#f7f8fa] p-4 sm:p-6"><div className="mb-5 flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-primary">Live operations</p><p className="mt-1 text-xl font-extrabold">Workshop overview</p></div><div className="grid size-10 place-items-center rounded-xl bg-[#172028] text-white"><Gauge /></div></div><div className="grid grid-cols-2 gap-3 sm:grid-cols-4">{[["Active jobs","17"],["Bookings","08"],["Approvals","03"],["Ready","03"]].map(([label,value], i)=><div key={label} className="rounded-xl border border-black/7 bg-white p-3"><div className={`mb-5 h-1.5 w-7 rounded-full ${["bg-orange-500","bg-sky-500","bg-amber-500","bg-emerald-500"][i]}`} /><p className="text-2xl font-black">{value}</p><p className="mt-1 text-xs text-muted-foreground">{label}</p></div>)}</div><div className="mt-3 rounded-2xl border border-black/7 bg-white p-4"><div className="flex items-center justify-between"><p className="font-bold">Repair pipeline</p><span className="text-xs font-semibold text-primary">Updated now</span></div><div className="mt-5 grid grid-cols-5 gap-2">{[["Inspect",4],["Approve",3],["Repair",5],["QC",2],["Ready",3]].map(([label,value],i)=><div key={label as string} className="text-center"><div className="mx-auto flex h-24 items-end rounded-lg bg-[#eef0f2] p-1"><div className={`w-full rounded-md ${["bg-sky-500","bg-amber-500","bg-orange-500","bg-violet-500","bg-emerald-500"][i]}`} style={{height:`${Number(value)*15}%`}} /></div><p className="mt-2 text-[10px] font-bold text-muted-foreground">{label}</p></div>)}</div></div></div></div>
        </div>
      </section>
      ...
      <footer className="border-t border-black/8 bg-white"><div className="mx-auto flex max-w-7xl flex-col justify-between gap-5 px-5 py-8 text-sm text-muted-foreground sm:flex-row lg:px-8"><p>© 2026 AutoCare Pro. Final Year Project.</p><div className="flex gap-5"><Link href="/sign-in">Sign in</Link><Link href="/dashboard">Workspace</Link><a href="#platform">Platform</a></div></div></footer>
    </main>
  )
}
